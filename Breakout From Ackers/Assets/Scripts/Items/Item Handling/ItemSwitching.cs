...
public class ItemSwitching : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else // Player has multiple equippable items
        {
            ...
            if(!(m1911.GetComponent<Gun>().isShooting || m1911.GetComponent<Gun>().isReloading || medKit.GetComponent<MedKit>().isHealing || pills.GetComponent<Pills>().isPopping) && !playerController.isDead) // || knife.GetComponent<Knife>().isAttacking
            {
